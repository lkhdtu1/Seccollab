...
def test_schedule_creation_with_email(auth_token):
    ...
    schedule_data = {
        ...
        "start_time": future_date.strftime("%Y-%m-%d %H:%M:%S"),
        "end_time": (future_date + timedelta(hours=1)).strftime("%Y-%m-%d %H:%M:%S"),
        "participants": ["testuser@example.com", "participant2@example.com"],
...
def test_schedule_cancellation_with_email(auth_token, schedule_id):
    ...
    try:
        response = requests.post(f"{BASE_URL}/schedules/{schedule_id}/cancel", headers=headers)
...
def test_server_health():
    ...
    endpoints_to_test = [
        "/health",
        "/api/status", 
        "/schedules"  # This should require auth
    ]
